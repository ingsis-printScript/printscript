...
class ReadInputArgumentRule(
    ...
    fun checkNodes(node: ASTNode) {
        when (node) {
            is ReadInputNode -> { checkReadInputArguments(node) }
    ...
    private fun checkReadInputArguments(readInputNode: ReadInputNode) {
        val value: String = readInputNode.prompt
        when (value) {
            ...
            is OptionalExpression.HasExpression -> {
                ...
                if (expression::class in prohibitedNodes) {
                    violations
                        .add(
                            LinterViolation(
                                "readInput() can not contain $expression",
                                readInputNode.range
                            )
                        )
                }
